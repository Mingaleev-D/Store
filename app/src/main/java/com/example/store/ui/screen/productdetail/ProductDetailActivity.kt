...
class ProductDetailActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    binding.apply {
      ...
      if (PrefUtils.getBagCount(item) > 0) {
        btnAddToBag.visibility = View.GONE
      }
      ...
      btnAddToBag.setOnClickListener {
        item.bagCount = 1
        PrefUtils.setBag(item)
        Toast.makeText(this@ProductDetailActivity, "Product add", Toast.LENGTH_SHORT).show()
        finish()
      }
    }
